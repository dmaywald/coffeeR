...
just_dates        = data.frame(t(jh_data_total_confirm %>% select(starts_with('X'))))
...
jh_data_total_confirm = select(jh_data_total_confirm, -contains('Princess'))


# Sum up states/territories for US total
jh_data_total_confirm$US = jh_data_total_confirm %>% select(-c(date, year, month, day_of_week)) %>% rowSums()
