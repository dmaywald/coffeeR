...
jh_data_daily_confirm = select(jh_data_daily_confirm, -contains('Princess'))


# Sum up states/territories for US total
jh_data_daily_confirm$US = jh_data_daily_confirm %>% select(-c(date, year, month, day_of_week)) %>% rowSums()
