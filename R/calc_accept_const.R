calc_accept_const <- function(eta_bounds, omega_bounds, phi_bounds, N = 25000, TS_data, kappa_const_dow, eta_coeff, num_test){
  ...
  f_x = f_x = sapply(1:N, function(num){inv_dist(x = c(temp[num,1], temp[num,2], temp[num,3]),
                                                 TS_data, kappa_const_dow, eta_coeff, num_test)})
